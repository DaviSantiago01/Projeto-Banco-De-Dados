...
async function carregarTodosOsDados(signal) {
  const [clientes, fornecedores, atendentes, categorias, produtos, vendas, logsAlteracaoPreco] =
    await Promise.all([
  ...
  return { clientes, fornecedores, atendentes, categorias, produtos, vendas, logsAlteracaoPreco };
}

async function carregarRelatorios(signal, filtros) {
  const resultados = await Promise.allSettled([
    buscarConsultaAtendentesPorVendas(Number(filtros.atendentesMinimoVendas || 0), signal),
  ...
  ] = resultados.map((resultado) => (resultado.status === 'fulfilled' ? resultado.value : []));
...
function Aplicacao() {
  ...
  const [formularioCliente, setFormularioCliente] = useState(montarFormularioInicialCliente());
  ...
  const [formularioProcedimentoPreco, setFormularioProcedimentoPreco] = useState({
    codigo: '',
    novoPreco: '',
  });
  ...
  const [filtrosRelatorios, setFiltrosRelatorios] = useState(montarFiltrosRelatoriosIniciais());
  ...
  const [consultandoMelhorFornecedor, setConsultandoMelhorFornecedor] = useState(false);
  const [executandoProcedimentoPreco, setExecutandoProcedimentoPreco] = useState(false);
  ...
  const [consultaMelhorPrazoFornecedor, setConsultaMelhorPrazoFornecedor] = useState([]);
    ...
    async function carregarDadosIniciais() {
      ...
      try {
        const [dados, relatorios] = await Promise.all([
          ...
          carregarRelatorios(controleAbortar.signal, montarFiltrosRelatoriosIniciais()),
        ]);
        ...
        const operacoesIniciais = montarOperacoesProdutoIniciais(dados.produtos);
        setCodigoConsultaFornecedor(operacoesIniciais.codigoConsultaFornecedor);
        setFormularioProcedimentoPreco(operacoesIniciais.formularioProcedimentoPreco);
        setFormularioVenda(
        ...
        setConsultaMelhorPrazoFornecedor(relatorios.consultaMelhorPrazoFornecedor);
  ...
  async function atualizarDados({
    ...
    try {
      ...
      const produtoOperacao =
        dados.produtos.find((produto) => produto.codigo === formularioProcedimentoPreco.codigo) ??
        dados.produtos.find((produto) => produto.codigo === codigoConsultaFornecedor) ??
        dados.produtos[0];
  ...
  function atualizarCampoCliente(evento) {
    const { name, value } = evento.target;
    setFormularioCliente((formularioAtual) => ({ ...formularioAtual, [name]: value }));
  }

  function atualizarCampoFornecedor(evento) {
    const { name, value } = evento.target;
    setFormularioFornecedor((formularioAtual) => ({ ...formularioAtual, [name]: value }));
  }
  ...
  function atualizarCampoProcedimentoPreco(evento) {
    const { name, value } = evento.target;
    setFormularioProcedimentoPreco((formularioAtual) => ({ ...formularioAtual, [name]: value }));
  }

  function atualizarFiltroRelatorio(evento) {
    const { name, value } = evento.target;
    setFiltrosRelatorios((filtrosAtuais) => ({ ...filtrosAtuais, [name]: value }));
  }
  ...
  async function salvarCliente(evento) {
    ...
    try {
      if (cpfClienteEmEdicao) {
        ...
        setAviso({ tipo: 'success', mensagem: 'Cliente atualizado com sucesso.' });
      } else {
  ...
  async function salvarFornecedor(evento) {
    ...
    try {
      if (cnpjFornecedorEmEdicao) {
        ...
        setAviso({ tipo: 'success', mensagem: 'Fornecedor atualizado com sucesso.' });
      } else {
        await criarFornecedor(dadosFornecedor);
        setAviso({ tipo: 'success', mensagem: 'Fornecedor criado com sucesso.' });
      }
  ...
  async function confirmarExclusao() {
    ...
    try {
      if (confirmacaoExclusao.tipo === 'produto') {
        ...
        await atualizarDados({ limparProduto: codigoProdutoEmEdicao === confirmacaoExclusao.id });
      }

      if (confirmacaoExclusao.tipo === 'cliente') {
        await excluirCliente(confirmacaoExclusao.id);
        setAviso({ tipo: 'success', mensagem: 'Cliente excluido com sucesso.' });
        setAbaAtiva('clientes');
        await atualizarDados({ limparCliente: cpfClienteEmEdicao === confirmacaoExclusao.id });
      }

      if (confirmacaoExclusao.tipo === 'fornecedor') {
        await excluirFornecedor(confirmacaoExclusao.id);
        setAviso({ tipo: 'success', mensagem: 'Fornecedor excluido com sucesso.' });
      ...
      if (confirmacaoExclusao.tipo === 'venda') {
        ...
        await atualizarDados({ limparVenda: numeroVendaEmEdicao === confirmacaoExclusao.id });
      }
  ...
  function iniciarEdicaoProduto(produto) {
    ...
    setAviso({ tipo: 'info', mensagem: `Edição ativa para o produto ${produto.codigo}.` });
  }
  ...
  function iniciarEdicaoCliente(cliente) {
    ...
    setAviso({ tipo: 'info', mensagem: `Edição ativa para o cliente ${cliente.cpf}.` });
  }
  ...
  function iniciarEdicaoFornecedor(fornecedor) {
    ...
    setAviso({ tipo: 'info', mensagem: `Edição ativa para o fornecedor ${fornecedor.cnpj}.` });
  }
  ...
  function iniciarEdicaoVenda(venda) {
    ...
    setAviso({ tipo: 'info', mensagem: `Edição ativa para a venda ${venda.numero}.` });
  }
  ...
  async function consultarMelhorFornecedor(evento) {
    ...
    if (!codigoConsultaFornecedor.trim()) {
      setAviso({ tipo: 'error', mensagem: 'Informe o código do produto para consultar a função.' });
      return;
    }
    ...
    try {
      const fornecedor = await buscarMelhorFornecedorProduto(codigoConsultaFornecedor.trim());
  ...
  async function executarProcedimentoPreco(evento) {
    ...
    try {
      ...
      setAviso({
        tipo: 'success',
        mensagem: 'Procedimento executado com sucesso e log de preço atualizado.',
      });
    } catch (erro) {
      setAviso({ tipo: 'error', mensagem: erro.message });
    } finally {
  ...
  async function executarConsultaAtendentesVendas(evento) {
    ...
    setAviso({ tipo: 'success', mensagem: 'Consulta de atendentes atualizada.' });
  }
  ...
  const contextoTela = {
    ...
    clientes: {
      titulo: 'Clientes',
      kicker: 'Cadastros',
      subtitulo: 'Cadastre, consulte e atualize os clientes usados nas vendas da loja.',
    },
    fornecedores: {
      titulo: 'Fornecedores',
      kicker: 'Cadastros',
      subtitulo: 'Gerencie os fornecedores cadastrados e mantenha a base comercial organizada.',
    },
    produtos: {
      titulo: 'Produtos',
      kicker: 'Catálogo',
      subtitulo: 'Controle o catálogo da loja e acompanhe consultas e operações complementares dos produtos.',
    },
    vendas: {
      titulo: 'Vendas',
      kicker: 'Operação diária',
      subtitulo: 'Registre, consulte e atualize as vendas realizadas no banco de dados.',
    },
    relatorios: {
      titulo: 'Relatórios',
      kicker: 'Views consolidadas',
      subtitulo: 'Visualize apenas os dados consolidados da base para apoiar a leitura geral do sistema.',
    },
  }[abaAtiva];
      ...
      <main className="app-shell">
        ...
        <div className="app-body">
          <aside className="sidebar-nav" aria-label="Modulos principais">
            <h2 className="sidebar-nav__title">Loja de Materiais de Construção</h2>
            <div className="sidebar-nav__content">
              <nav className="sidebar-nav__list" role="tablist" aria-orientation="vertical">
                {ABAS.map((aba) => (
                  <Fragment key={aba.id}>
                    {aba.separadoAnterior ? <hr className="sidebar-separator" /> : null}
                    <button
                      id={`tab-${aba.id}`}
                      type="button"
                      role="tab"
            ...
            <section className="workspace" id="conteudo-principal">
              {abaAtiva === 'dashboard' ? (
                ...
                <SecaoClientes
                  ...
                  aoExecutarConsultaClientesPorNome={executarConsultaClientesPorNome}
                ...
                <SecaoProdutos
                  ...
                  aoMudarCampoProcedimentoPreco={atualizarCampoProcedimentoPreco}
                ...
                <SecaoRelatorios
                  ...
                  filtrosRelatorios={filtrosRelatorios}
                  aoAtualizarFiltroRelatorio={atualizarFiltroRelatorio}
                  aoExecutarConsultaAtendentesVendas={executarConsultaAtendentesVendas}
                  viewVendasDetalhadas={viewVendasDetalhadas}
